-- ==============================================
-- AI BOOKKEEPER ASSISTANT - SQLite Schema
-- Compatible con tu configuración existente
-- ==============================================
...
CREATE TABLE IF NOT EXISTS ocr_processing_log (
    ...
    document_type TEXT CHECK(document_type IN ('auto', 'invoice', 'bank_statement', 'receipt', 'general')) NOT NULL,
    client_id TEXT,
    status TEXT CHECK(status IN ('queued', 'processing', 'completed', 'failed')) NOT NULL DEFAULT 'queued',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    extraction_options TEXT, -- JSON as TEXT in SQLite
...
CREATE TABLE IF NOT EXISTS ocr_extraction_results (
    ...
    raw_text TEXT,
    structured_data TEXT, -- JSON as TEXT in SQLite
    confidence REAL,
...
CREATE TABLE IF NOT EXISTS bank_reconciliations (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    options TEXT -- JSON as TEXT in SQLite
);
...
CREATE TABLE IF NOT EXISTS reconciliation_matches (
    ...
    confidence REAL NOT NULL,
    match_type TEXT CHECK(match_type IN ('exact', 'high_confidence', 'medium_confidence', 'low_confidence')) NOT NULL,
    differences TEXT, -- JSON as TEXT in SQLite
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS reconciliation_discrepancies (
    ...
    type TEXT CHECK(type IN ('unmatched_bank', 'unmatched_book', 'low_confidence_match')) NOT NULL,
    description TEXT NOT NULL,
    impact_amount REAL NOT NULL,
    severity TEXT CHECK(severity IN ('low', 'medium', 'high')) NOT NULL,
    suggested_actions TEXT, -- JSON as TEXT in SQLite
    resolved INTEGER DEFAULT 0, -- BOOLEAN as INTEGER in SQLite
...
CREATE TABLE IF NOT EXISTS document_patterns (
    ...
    pattern_data TEXT NOT NULL, -- JSON as TEXT in SQLite
...
CREATE TABLE IF NOT EXISTS client_ocr_config (
    ...
    chart_of_accounts TEXT, -- JSON as TEXT in SQLite
    categorization_rules TEXT, -- JSON as TEXT in SQLite
    ocr_preferences TEXT, -- JSON as TEXT in SQLite
    auto_categorize INTEGER DEFAULT 1, -- BOOLEAN as INTEGER in SQLite
    auto_reconcile INTEGER DEFAULT 0, -- BOOLEAN as INTEGER in SQLite
...
INSERT OR IGNORE INTO client_ocr_config (client_id, chart_of_accounts, categorization_rules, ocr_preferences) 
VALUES (
    'default',
...
SELECT 'Tablas OCR creadas correctamente. Total de tablas:' as message, COUNT(*) as count
FROM sqlite_master 
WHERE type = 'table' 
AND name LIKE '%ocr%' OR name LIKE '%reconcil%' OR name LIKE '%categoriz%' OR name LIKE '%document%';

-- Mostrar todas las tablas relacionadas con OCR
